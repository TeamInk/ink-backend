...
public class MemberAuthController {
    ...
    @PostMapping("/signin")
    public ResponseEntity<ApiResult<TokenDto.Ink>> signin(
            @ApiParam(value = "액세스 토큰", required = true) @RequestBody @Valid TokenDto.Provider providerToken) {
        return ResponseEntity.ok(ApiResult.ok(
    ...
    @PostMapping("/refresh")
    public ResponseEntity<ApiResult<TokenDto.Ink>> refresh(
            @ApiParam(value = "액세스 토큰", required = true) @RequestBody @Valid TokenDto.InkRefresh inkRefreshToken) {
        return ResponseEntity.ok(ApiResult.ok(
                jwtService.refresh(inkRefreshToken)));
    }
